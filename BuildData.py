...
def BuildTripleFile(txtdir,name,ids):
            ...
            for j in xrange(0,2):
                ...
                for n in xrange(0,eachlet):
                    aablist.append(['CamA_%04d_%02d.jpg' %(id+1,i+1),\
									...
									'CamB_%04d_%02d.jpg' %(tmpneg[n]+2,np.random.randint(0,2)+1)])
            ...
            for j in xrange(0,2):
                ...
                for n in xrange(0,eachlet):
                    bbalist.append(['CamB_%04d_%02d.jpg' %(id+1,i+1),\
									...
									'CamA_%04d_%02d.jpg' %(tmpneg[n]+2,np.random.randint(0,2)+1)])
    ...
    neg1_list = open(txtdir+'/CUHK01_%s_neg1.txt' %name, 'w')
    for x in range(0,list.shape[0]):
        neg1_list.write('%s 0\n' %list[x,2])
    neg1_list.close()
    
    neg2_list = open(txtdir+'/CUHK01_%s_neg2.txt' %name, 'w')
    for x in range(0,list.shape[0]):
        neg2_list.write('%s 0\n' %list[x,3])
    neg2_list.close()
...
if __name__ == '__main__':
    ...
    txtdir = '/home/n9346490/Quartet/CUHK01Data'
    #output path for three folders (train,val)
    outdir = '/home/n9346490/Quartet/CUHK01Data/image'
    valdir ='/home/n9346490/Quartet'
    #the CUHK01 source data
    indir='/work/SAIVT/CUHK01/campus'
    ...
    val_ratio=0.5
    (val_data, train_data) = CreateTrainVal(all_data,val_ratio)
